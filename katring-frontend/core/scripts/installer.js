...
const STOREFRONT_BACKEND_GIT_URL = 'git@github.com:HikvIneH/vue-storefront-api.git'
...
const KATRING_LOG_FILE = `${STOREFRONT_DIRECTORY}/var/log/vue-storefront.log`
const KATRING_BACKEND_LOG_FILE = `${STOREFRONT_DIRECTORY}/var/log/vue-storefront-api.log`
  ...
  runDevEnvironment () {
    return new Promise((resolve, reject) => {
      ...
      if (shell.exec(`nohup npm run dev > ${Abstract.backendLogStream} 2>&1 &`).code !== 0) {
        reject(new Error('Can\'t start dev server.', KATRING_BACKEND_LOG_FILE))
      }
  ...
  npmBuild () {
    return new Promise((resolve, reject) => {
      ...
      if (shell.exec(`npm run build > ${Abstract.storefrontLogStream} 2>&1`).code !== 0) {
        reject(new Error('Can\'t build storefront npm.', KATRING_LOG_FILE))
      }
  ...
  runDevEnvironment (answers) {
    return new Promise((resolve, reject) => {
      ...
      if (shell.exec(`nohup npm run dev >> ${Abstract.storefrontLogStream} 2>&1 &`).code !== 0) {
        reject(new Error('Can\'t start storefront server.', KATRING_LOG_FILE))
      }
  ...
  tryToCreateLogFiles () {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        Abstract.storefrontLogStream = KATRING_LOG_FILE
        Abstract.backendLogStream = KATRING_BACKEND_LOG_FILE
      } catch (e) {
...
let questions = [
  ...
  },
  {
    ...
    default: 'q8f0x2rdk56qobuljxigr9ms5tddoqiq',
    when: function (answers) {
      return answers.m2_api_oauth2 === true
    }
  },
  {
    ...
    default: '2lg2msi8osmdaiv34r3rm8m0ghtohcvf',
    when: function (answers) {
      return answers.m2_api_oauth2 === true
    }
  },
  {
    ...
    default: 'tspqof20w2319elaw2xpl1ug9acy6dci',
    when: function (answers) {
      return answers.m2_api_oauth2 === true
    }
  },
  {
    ...
    default: 'kklg15n1j5bk5206g5vtejiljymdkx4t',
    when: function (answers) {
      return answers.m2_api_oauth2 === true
    }
  }
